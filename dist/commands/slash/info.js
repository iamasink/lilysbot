...
export default {
    data: new SlashCommandBuilder()
        ...
        .addChoices({ name: 'avatar', value: 'avatar' }, { name: 'guild avatar', value: 'guild avatar' }, { name: 'banner', value: 'banner' }, { name: 'hide', value: 'hide' })))
        .addSubcommand((subcommand) => subcommand
        .setName('guild')
        ...
        switch (interaction.options.getSubcommand()) {
            case 'user': {
                ...
                if (user.avatarURL(true)) {
                    ...
                    thumb = `https://cdn.discordapp.com/embed/avatars/${user.discriminator % 5}.png`;
                }
                ...
                if (interaction.options.getString('show') != 'hide') {
                    ...
                    guildtext += `\n**Invited by:** ${invited}`;
                    if (member)
                        infoEmbed.addFields({ name: '__Guild__', value: guildtext });
                    if (image)
                        infoEmbed.addFields({ name: `\u200b`, value: `**Showing ${imagename}:**` });
                    if (image)
                        infoEmbed.setImage(`${image}?size=4096`);
                }
