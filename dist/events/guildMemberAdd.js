...
exports.default = {
    ...
    async execute(member) {
        ...
        const inviter = await index_1.client.users.fetch(invite.inviterId);
        // set inviter code for member
        database.set(`.guilds.${member.guild.id}.users.${member.id}.invitedLink`, invite.code);
        const inviterUser = await index_1.client.users.fetch(inviter.id);
            ...
            const interaction = await commands.run(msg, "info", null, "user", [
                {
                    ...
                    type: 6,
                    value: member.id,
                    user: await index_1.client.users.fetch(member.id),
