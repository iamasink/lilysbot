...
export default new ApplicationCommand({
	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((command) =>
			command.setName("leave").setDescription("leave"),
		)
		.addSubcommand((command) =>
			command.setName("join").setDescription("join"),
		),
		...
		switch (interaction.options.getSubcommand()) {
			case "play": {
				...
				const audioResource = createAudioResource(
					"/mnt/storage/media/data/media/music/Porter Robinson/Nurture/15 - fullmoon lullaby.flac",
					{
						metadata: {
							title: "A good song!",
						},
