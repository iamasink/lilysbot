	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		switch (interaction.options.getSubcommand()) {
			...
			case "guild": {
				...
				const textChannelCount = channels.filter((i) =>
					i.isTextBased(),
				).size
				const voiceChannelCount = channels.filter((i) =>
					i.isVoiceBased(),
				).size
				...
				const threadChannelCount = channels2.filter((i) =>
					i.isThread(),
				).size

				const roles = await guild.roles.fetch()
				const roleCount = roles.size
				const managedRoleCount = roles.filter(
					(role) => role.managed,
				).size
				...
				const botMemberCount = members.filter(
			...
			case "bot": {
				...
				const infoEmbed = new EmbedBuilder()
					...
					.setThumbnail(
						`${user.avatarURL({ forceStatic: false })}?size=4096`,
					)
					//.setAuthor({ name: 'Some name', iconURL: 'https://i.imgur.com/AfFp7pu.png', url: 'https://discord.js.org' })
					.setDescription(
						`**ID**: ${
