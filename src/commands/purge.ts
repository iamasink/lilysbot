...
export default new ApplicationCommand({
	...
	async execute(interaction: ChatInputCommandInteraction): Promise<void> {
		...
		try {
			channel.bulkDelete(newMessages, true).then(async (m) => {
				let note = ""
				if (m.size < amount)
					note = `*Note: There weren't enough recent messages matching the parameters to delete **${amount}***`
				await interaction.reply(
					`Bulk deleted **${m.size}** messages.\n${note}`,
				)
				log.log(
					interaction.guild,
					`${interaction.user} bulkdeleted ${m.size} messages in ${interaction.channel}`,
				)
