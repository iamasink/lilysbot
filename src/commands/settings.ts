	...
	async execute(interaction) {
		...
		switch (interaction.options.getSubcommand()) {
			case 'set': {
				...
				const oldvalue = await settings.get(interaction.guild, settingName)
				let value = ""
				switch (option.type) {
					case 'channel': {
						let channel = await channelSelector(interaction, option.name, oldvalue)
						if (channel) {
					...
					case 'role': {
						let role = await roleSelector(interaction, option.name, oldvalue)
						if (role) {
					...
					case 'toggle': {
						value = await toggleSelector(interaction, option.name, oldvalue)
						break
					}
					case 'string': {
						value = await stringSelector(interaction, option.name, option.options, oldvalue)
						break
					}
					// case 'list': {
...
async function channelSelector(interaction: ChatInputCommandInteraction, setting: string, oldvalue: string): Promise<TextChannel> {
	...
	return new Promise(async (resolve, reject) => {
		...
		await interaction.reply({ embeds: embeds.messageEmbed(title, `**Currently**: *<#${oldvalue}>*`), components: [row, row2], ephemeral: true }).then((message) => {
			message
				...
				.then(async (i) => {
					...
					if (i.componentType == ComponentType.ChannelSelect) {
						...
						if (channel.type == ChannelType.GuildText) {
							interaction.editReply({ embeds: embeds.successEmbed(title, `You selected <#${channel.id}>`), components: [] })
							resolve(channel)

						} else {
							interaction.editReply({ embeds: embeds.messageEmbed(title, `Invalid channel selected:\n${channel.name}`, null, "#ff0000"), components: [] })
						...
						let channel = null
						console.log(channel)
						interaction.editReply({ embeds: embeds.successEmbed(title, `You selected none`), components: [] })
						resolve(channel)
					}

				})
				.catch(err => {
					interaction.editReply({ embeds: embeds.messageEmbed(title, `No channel selected`, null, "#ff0000"), components: [] })
...
async function roleSelector(interaction: ChatInputCommandInteraction, setting: string, oldvalue: string): Promise<Role> {
	...
	return new Promise(async (resolve, reject) => {
		...
		await interaction.reply({ embeds: embeds.messageEmbed(title, `**Currently**: *${interaction.guild.roles.fetch(oldvalue)}*`), components: [row, row2], ephemeral: true }).then((message) => {
			message
				...
				.then(async (i) => {
					if (i.componentType == ComponentType.RoleSelect) {
						...
						console.log(role)
						interaction.editReply({ embeds: embeds.successEmbed(title, `You selected <@&${role.id}>`), components: [] })
						resolve(role)
					}
					else if (i.componentType == ComponentType.Button) {
						let role = null
				...
				.catch(err => {
					interaction.editReply({ embeds: embeds.messageEmbed(title, `No role selected`, null, "#ff0000"), components: [] })
...
async function toggleSelector(interaction: ChatInputCommandInteraction, setting: string, oldvalue: string): Promise<any> {
	return stringSelector(interaction, setting, [{ label: "On", value: "true" }, { label: "Off", value: "false" }], oldvalue == "true" ? "On" : "Off")
}

async function stringSelector(interaction: ChatInputCommandInteraction, setting: string, options: SelectorOption[], oldvalue: string): Promise<any> {
	...
	return new Promise(async (resolve, reject) => {
		...
		await interaction.reply({ embeds: embeds.messageEmbed(title, `**Currently**: *${oldvalue}*`), components: [row], ephemeral: true }).then((message) => {
			message
				.awaitMessageComponent({ filter, componentType: ComponentType.StringSelect, time: 60000 })
				.then(async (i) => {
					let value = i.values[0]
					console.log(value)
					interaction.editReply({ embeds: embeds.successEmbed(title, `You selected ${value}`), components: [] })
					resolve(value)
				})
				.catch(err => {
