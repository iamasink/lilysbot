	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((command) =>
			command
				.setName("get")
				.setDescription(
					"get info about a torrent. attach a .torrent file or enter a magnet link",
				)
				...
				.addBooleanOption((option) =>
					option
						.setName("hide")
						.setDescription(
							"hide the information for everyone but you. defaults to true. recommended for private torrents",
						),
				),
		),
				...
				if (attachment) {
					parseTorrent.remote(
						...
						async (err, parsedTorrent) => {
							...
							var extrainfo = ``
							if ((torrent.info as any).comment)
								extrainfo += `\n**Comment:** ${
									(torrent.info as any).comment
								}`
					...
					try {
						...
						if (torrent.dn)
							extrainfo += `\nDisplay Name: ${Array.from(
								[torrent.dn].flat(),
							).join("\n	-	")}`
						if (torrent.tr)
							extrainfo += `\nAddress TRacker: ${Array.from(
								[torrent.tr].flat(),
							).join("\n	-	")}`
						if (torrent.xs)
							extrainfo += `\neXact Source: ${Array.from(
								[torrent.xs].flat(),
							).join("\n	-	")}`
						if (torrent.as)
							extrainfo += `\nAcceptable Source: ${Array.from(
								[torrent.as].flat(),
							).join("\n	-	")}`
						if (torrent.ws)
							extrainfo += `\nWeb Seed: ${Array.from(
						...
						if (torrent.keywords)
							extrainfo += `\nkeywords: ${Array.from(
								[torrent.keywords].flat(),
							).join("\n	-	")}`
						if (torrent.announce)
							extrainfo += `\nannounce: ${Array.from(
								[torrent.announce].flat(),
							).join("\n	-	")}`
						if (torrent.urlList)
							extrainfo += `\nurlList: ${Array.from(
								[torrent.urlList].flat(),
							).join("\n	-	")}`
					...
					interaction.editReply({
						embeds: embeds.warningEmbed(
							"No Torrent Selected",
							"You must add an attachment to a .torrent file or a magnet link",
						),
					})
				}
