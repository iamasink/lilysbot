	...
	async execute(member: GuildMember) {
		...
		if (!kickLog) {
			...
			if (target.id === member.id) {
				log.log(member.guild, `${member.user.tag} left the guild; kicked by ${executor.tag}?`);
				console.log(`${member.user.tag} left the guild; kicked by ${executor.tag}?`);
				action = "was kicked"
			} else {
				console.log(`${member.user.tag} left the guild, audit log fetch was inconclusive.`);
				log.log(member.guild, `${member.user.tag} left the guild, audit log fetch was inconclusive.`);
				action = "vanished"
			}
		}
