import { AttachmentBuilder, GuildTextBasedChannel, Message, Snowflake, TextBasedChannel } from 'discord.js';
...
class BridgeManager {
    ...
    addBridge(channel1: Snowflake, channel2: Snowflake) {
        ...
        console.log(`Bridge created between channels ${channel1} and ${channel2}`);
        this.saveBridges()
    }
    ...
    removeBridge(channel1: Snowflake, channel2?: Snowflake) {
        let bridgedBridge
        if (!channel2) {
            bridgedBridge = this.bridges.find(bridge => bridge.isPartOfBridge(channel1));
        } else {
            bridgedBridge = this.bridges.find(bridge => bridge.channel1 === channel1 && bridge.channel2 === channel2);
        }
    ...
    saveBridges() {
        ...
        for (let i = 0, len = this.bridges.length; i < len; i++) {
            bridgedata.push({ channel1: this.bridges[i].channel1, channel2: this.bridges[i].channel2 })
        }
    ...
    async handleBridgedMessage(message: Message): Promise<void> {
        ...
        const bridgedBridge = this.bridges.find(bridge => bridge.isPartOfBridge(message.channel.id));
        if (bridgedBridge) {
            try {

                const oppositeChannelid = bridgedBridge.getOppositeChannel(message.channel.id);
                if (oppositeChannelid) {
                    // Logic to handle the opposite channel
                    console.log(`Message is in a bridged channel. Opposite channel ID: ${oppositeChannelid}`);
                    const oppositeChannel = (await client.channels.fetch(oppositeChannelid) as TextBasedChannel)
                    ...
                    for (let i = 0, len = msgs.length; i < len; i++) {
                        ...
                        if (i == msgs.length - 1) { // attach files only to the last one
                            webhooks.send((oppositeChannel as GuildTextBasedChannel),
                                {
                                    content: msgs[i],
                                    username: `${message.member.displayName || message.author.username} (bridged by Wiwwie)`,
                                    files: files,
